                ...
                function neo4jDataToD3Data(data) {
                    ...
                    data.forEach(function (data) {
                        data.nodes.forEach(function (node) {
                            if (!contains(graph.nodes, node.id) && !contains(nodes, node.id)) {
                                graph.nodes.push(node);
                            }
                        });

                        data.relationships.forEach(function (relationship) {
                            if (!contains(graph.relationships, relationship.id) && !contains(relationships, relationship.id)) {
                ...
                function tickRelationshipsOutlines() {
                    relationship.each(function (relationship) {
                        var rel = d3.select(this),
                            ...
                            padding = 0;// ARIF: in order to remove the text empty space

                        outline.attr('d', function (d) {
                            var center = {x: 0, y: 0},
                                angle = rotation(d.source, d.target),
                                textBoundingBox = text.node().getBBox(),
                                textPadding = 0,// ARIF: in order to remove the text empty space
